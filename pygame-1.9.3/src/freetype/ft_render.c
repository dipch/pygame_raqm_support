       ...
       FT_Pos underline_top, FT_Fixed underline_size)
{
    ...
    for (n = 0; n < length; ++n) {
        ...
        x = FX6_TRUNC(FX6_CEIL(left + slots[n].posn.x + slots[n].offset.x));//x = FX6_TRUNC(FX6_CEIL(left + slots[n].posn.x));
        y = FX6_TRUNC(FX6_CEIL(top + slots[n].posn.y - slots[n].offset.y));//y = FX6_TRUNC(FX6_CEIL(top + slots[n].posn.y));
