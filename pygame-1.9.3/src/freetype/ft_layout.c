          ...
          const PGFT_String *text)
{
    ...
    FT_Fixed y_scale = sz_metrics->y_scale;//int have_kerning = FT_HAS_KERNING(font);//Py_ssize_t length = 0;
    GlyphSlot *slots;//GlyphIndex_t id; GlyphIndex_t prev_id = 0; FT_UInt32 ch;
    Py_ssize_t i;//FT_Error error = 0;
    ...
    /* create the text struct */ //string_length
    if (nglyphs > ftext->buffer_size) {
        _PGFT_free(ftext->glyphs);
        ftext->glyphs = (GlyphSlot *)
            _PGFT_malloc((size_t) nglyphs * sizeof(GlyphSlot));//string_length
        if (!ftext->glyphs) {
            PyErr_NoMemory();
            goto cleanup;//return -1;
        }
        ftext->buffer_size = nglyphs;//string_length;
    }
...
position_glyphs(Layout *ftext)
{
    ...
    FT_Vector   pen = {0, 0};                /* untransformed origin  */ //FT_Vector   pen1 = {0, 0}; //FT_Vector   pen2;
    int         vertical = ftext->mode.render_flags & FT_RFLAG_VERTICAL;
    ...
    for (i = 0; i != n_glyphs; ++i) {
        ...
        if (vertical) {
            ...
            pen.y -= slot->v_advance.y ;//pen.y += metrics->advance_rotated.y;
        }
        else {
            if (pen.y - metrics->bearing_rotated.y < min_y) {
                min_y = pen.y - metrics->bearing_rotated.y;
            ...
            slot->posn.y = pen.y - metrics->bearing_rotated.y;
            pen.y -= slot->h_advance.y;//pen.y -= metrics->advance_rotated.y;
        }
...
{
    FontCache *cache = &(fontobj->_internals->glyph_cache);//FT_UInt32 ch = (FT_UInt32)character;
    GlyphIndex_t id;
    FontGlyph *glyph = 0;
    TextContext context;
    FT_Face     font;
    ...
    fill_context(&context, ft, fontobj, mode, font);
    id = glyphs.index;//id = FTC_CMapCache_Lookup(context.charmap, context.id, -1, ch);
    if (!id) {
        return -1;
    }
    ...
    *advance_x = (double)(h_advance.x / 64.0);//*advance_x = (double)(glyph->h_metrics.advance_rotated.x / 64.0);
    *advance_y = (double)(h_advance.y / 64.0);//*advance_y = (double)(glyph->h_metrics.advance_rotated.y / 64.0);
                ...
                const FontRenderMode *mode, void *internal)
{
    ...
    if (rotation_angle == 0) {
        v_bearing_rotated.x = -strong_delta.x / 2;//v_bearing_rotated.x = ft_metrics->vertBearingX - strong_delta.x / 2;
        v_bearing_rotated.y = 0;//v_bearing_rotated.y = ft_metrics->vertBearingY;
    }
    else {
        /*
